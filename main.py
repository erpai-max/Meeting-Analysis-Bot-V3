...
import yaml
import logging
import json   # ✅ FIXED: added import json
import time
...
def authenticate_google(config):
    """Authenticates with Google Drive + Sheets using service account key."""
    ...
    try:
        ...
        creds_info = json.loads(gcp_key_str)  # ✅ fixed, now json is imported
        scopes = [
...
def export_data_for_dashboard(gsheets_client, config):
    """Fetches all data from the results sheet and saves it as dashboard_data.json."""
